...
use super::{
    gen_config_builder::TwilightGenConfigBuilder, prompt_builder::TwilightPromptBuilder,
    session::TwilightSession,
};

#[derive(Debug, Error)]
pub enum TwilightModelError {
...
#[derive(Debug)]
pub struct TwilightModel {
    inner: LlamaModel,
    backend: LlamaBackend,
}

#[derive(Debug)]
pub struct TwilightModelResult {
    content: String,
    input_tokens: u32,
    out_tokens: u32,
}
...
impl TwilightModel {
    pub fn from_file(path: &str) -> anyhow::Result<TwilightSession> {
        let backend = LlamaBackend::init()?;
        let params = LlamaModelParams::default();
        let inner = LlamaModel::load_from_file(&backend, path, &params)?;
        let model = Box::leak(Box::new(TwilightModel { inner, backend }));
        Ok(TwilightSession::new(model))
    }
    ...
    pub(crate) fn generate(
        &self,
        prompt_builder: TwilightPromptBuilder,
        config_builder: TwilightGenConfigBuilder,
    ) -> anyhow::Result<TwilightModelResult> {
        println!("prompt:\n {}", &prompt_builder.content);
        let tokenized_prompt = self
            .inner
            .str_to_token(&prompt_builder.content, AddBos::Always)?;
        ...
        while cursor < total_required_tokens as i32 {
            ...
            if is_higher_than_max_tokens {
                println!("⚠️ Context limit reached ({} tokens)", ctx.n_ctx());
                break;
            }
            ...
            batch.add(token, cursor as i32, &[0], true)?;
            //get new logits
            ctx.decode(&mut batch)?;
